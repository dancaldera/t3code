...
export function useHandleNewThread() {
  ...
  const stickyProvider = useComposerDraftStore((store) => store.stickyProvider);
  ...
  const handleNewThread = useCallback(
    (
      ...
      return (async () => {
        ...
        if (stickyModel) {
          setProvider(threadId, stickyProvider);
    ...
    [navigate, routeThreadId, stickyModel, stickyModelOptions, stickyProvider],
  );
